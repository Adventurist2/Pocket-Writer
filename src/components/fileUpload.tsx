...
export default function FileUpload() {
    ...
    return (
        ...
        <button
            className="cursor-pointer fixed top-4 right-4 sm:top-10 sm:right-10 bg-red-600 hover:bg-red-500 text-white font-semibold p-2 rounded-full w-8 h-8 sm:w-10 sm:h-10 flex justify-center items-center shadow-md transition duration-300 ease-in-out transform hover:scale-110"
            onClick={(e) => {
                e.preventDefault();
                router.push("/");
            }}
        ...
        <div className="w-full max-w-lg sm:max-w-2xl bg-gray-800 rounded-xl p-4 sm:p-6 shadow-lg">
            <h2 className="text-lg font-semibold mb-4 text-center">Upload Files</h2>
    
            <form onSubmit={fetchImage} className="flex flex-col sm:flex-row items-center gap-4 mt-4">
                ...
                <input
                    ...
                    className="w-full sm:w-64 p-2 rounded-lg border border-gray-600 bg-gray-800 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button
                    type="submit"
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <div
                    className="w-full sm:w-1/2 h-[12rem] sm:h-[15rem] border-2 border-dashed border-gray-500 rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-700"
                ...
                >
                    ...
                    />
                    <svg className="w-10 h-10 sm:w-12 sm:h-12 text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <button 
                        className="mt-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-sm"
                ...
                <div className="w-full sm:w-1/2 h-[12rem] sm:h-[15rem] bg-gray-700 rounded-lg p-4 sm:p-6 flex items-center justify-center overflow-y-scroll overflow-x-hidden">
                    {files.length > 0 ? (
                        <div>
                            {files.map((file, index) => (
                                <FileBox key={index} index={index} s={file.name ?? "Untitled"} />
                            ))}
                        </div>
                    ) : (
            ...
            <div className="mt-4">
                <div className="flex gap-4 items-center">
                    ...
                    {caption && <p className="text-xs text-green-500">Caption Submitted</p>}
                </div>
                <textarea 
                    value={captionInput} 
                    className="w-full p-2 rounded-lg bg-gray-700 border border-gray-600 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none h-24 overflow-y-auto" 
                    placeholder="Enter caption..." 
                    onChange={handleCaptionChange}
                ></textarea>
                <button
                    className="cursor-pointer px-4 py-2 mt-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105"
            ...
            <form onSubmit={handleSubmit} className="mt-4 flex justify-center">
                <button className="cursor-pointer px-6 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-sm">
                    Submit
                </button>
            </form>
        </div>
